...
--> 위 2개의 단계를 하나의 쿼리로! --> 1번의 쿼리문을 서브쿼리로
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
--> 위 2단계를 하나의 쿼리로 만든다 --> 1번 쿼리를 서브쿼리로
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
 ...
 *                         메인쿼리 테이블의 값이 변경되면, 서브쿼리 결과값도 바뀌는 서브쿼리
 * 
 * - 스칼라 서브쿼리 : 상관 커리이면서 결과값이 하나인 서브쿼리
 * 
 ...
 * 
 * - IN / NOT IN : 여러 개의 결과값 중에 한 개라도 일치하는 값이 있다면
 ...
 * 
 * - EXISTS / NO EXISTS : 값이 존재하는가? / 존재하지 않는가? 
 * 
 */
...
-- 2) 직원의 사번, 이름, 부서명, 직급명 조회(구분 일단 제외)
...
-- 4) 일반 직원에 해당하는 사원들 정보 조회(구분은 '직원'으로)
...
-- 3) 대리 직급의 직원들 중에서 과장 직급의 최소 급여보다 많는 직원
...
FROM EMPLOYEE
WHERE ENT_YN = 'Y'
AND SUBSTR(EMP_NO, 8, 1)  = '2'; -- 1행(단일행) 이태림 D8 J6
 ...
 * 실제 데이터 값을 반환하지는 않음.
 * 즉, 각 직원의 MANAGER_ID가 다른 직원의 EMP_ID와 일치하는 경우만 확인함
 * 
...
-- 메인쿼리(사번, 이름, 부서코드, 부서명(NULL이면 '소속없음'), 직급명, 입사일 / 단, 퇴사 직원 제외)
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1';
/* 1. 메인쿼리의 첫번째 행인 방명수가 있는 부서 D1의 가장 빠른 입사일 조회 시
 *    2007-03-20 00:00:00.000 조회됨
