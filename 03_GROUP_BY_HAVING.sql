...
-- 그룹함수는 단 한개의 결과값만 산출하기 때문에 그룹이 여러개일 경우 오류 발생할 수 있다.
...
FROM 
    EMPLOYEE
GROUP BY 
    DEPT_CODE; --> DEPT CODE가 같은 행끼리 하나의 그룹이됨
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 --> 한사람의 급여가 300만 이상이라는 조건(요구 사항 X)
GROUP BY DEPT_CODE 
...
-- CUBE : GROUP BY절에 작성된 모든 컬럼의 중간 집게를 처리하는 함수
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
