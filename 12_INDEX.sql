 ...
 *   테이블의 특정 컬럼에 대한 색인이나 순서를 생성하는 객체
 * 
 * 
 * 
 ...
 * - 인덱스도 하나의 객체이기 때문에 이를 저장하기 위한 별도 공간이 필요.
 * - 인덱스 생성 시간이 필요
 * 
 * 
 * 3. 인덱스 생성 방법
...
WHERE EMP_ID = 215; -- 인덱스 사용 O (EMP_ID : PK)

SELECT * FROM EMPLOYEE
WHERE EMP_NAME = '대북혼'; -- 인덱스 사용 X
-- EMPLOYEE 테이블은 데이터가 많이 없어 속도 차이 체감 X
...
-- PL/SQL : 오라클에서 제공하는 프로그래밍 언어, 절차적 데이터 처리할 때 사용, FOR문 같이 사용
BEGIN
	FOR I IN 1..1000000
	LOOP 
...
WHERE TEST_ID = 'TEST500000'; -- 인덱스 X, 0.013s

SELECT * FROM TB_IDX_TEST
WHERE TEST_NO = 500000; -- 인덱스 O, 0.001s
