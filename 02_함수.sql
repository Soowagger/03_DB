...
/* * 함수(FUNCTION) : 컬럼의 값을 읽어서 연산한 결과를 반환
 * 
 * - 단일 행 함수 : N개의 값을 읽어 N개의 결과를 반환
 * 
 * - 그룹 합수 : N개의 값을 읽어 1개의 결과를 반환(합계, 평균, 최대, 최소) 
 * 
 * - 함수는 SELECT문 내 아래의 절에서 사용 가능
 * SELECT절
 * WHERE절
 * GROUP BY절
 * HAVING절
...
-- 문자열을 5번째 문자부터 검색하여 첫 번째에 있는 B위치를 조회
SELECT INSTR('AABAACAABBAA', 'B', 5) -- 9
FROM DUAL;

-- 문자열을 5번째 문자부터 검색하여 두 번째에 있는 B위치를 조회
...
-- EMPLLOYE 테이블에서 사원명, 이메일 중 아이디 부분만 조회
...
--> 소수점 몇 번째 자리까지 표현할거냐라고 생각하면 편함

SELECT ROUND(123.456, 0) FROM DUAL; -- 123, 소수점 첫 번째 자리에서 반올림(0 기본값)

SELECT ROUND(123.456, -1) FROM DUAL; -- 120, 소수점 0번째 반올림 -> 소수점 -1 자리 표현
...
-- 2024-02-29 11:21:05.582 +0900 (UTC에서 +9시간 나타냄, 한국 표준시)
...
FROM DUAL; 
-- -3.88787223715651135005973715651135005974
--> ROUND 사용 -3.888
...
--> 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인식시켜야함
...
SELECT '1,000,000' + 50000000 FROM DUAL;
...
DECODE(JOB_CODE, 'J7', '20%', 'J6', '15%', 'J5', '10%', 'J4', '5%', 'J3', '5%', 'J2','5%', 'J1', '5%') " 인상률",
DECODE(SUBSTR(JOB_CODE, 2, 1), '7', SALARY * 1.2, '6', SALARY * 1.15, '5', SALARY * 1.1, SALARY * 1.05) "인상된 급여"
--																																													그 외
FROM EMPLOYEE;
...
SELECT 
		MIN(SALARY), -- 숫자 1380000
		MIN(HIRE_DATE),  -- 날짜 1990-02-06 00:00:00.000
		MIN(EMAIL) -- 문자 bang_ms@or.kr
FROM EMPLOYEE;
