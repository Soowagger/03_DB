 ...
 * - 조회된 행들의 집합을 'RESULT SET'(조회결과의 집합)이라고 함.
 * 
...
-- 1. EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여, 연봉(급여*12) 조회
SELECT EMP_ID, EMP_NAME, SALARY, SALARY * 12 FROM EMPLOYEE;

SELECT ㅡMP_NAME + 10 FROM EMPLOYEE;
-- ORA-01722: 수치가 부적합합니다
-- 산술 연산은 숫자(NUMBER 타입)만 가능하다!

--------------------------------------------------------------------

-- 2. EMPLOYEE 테이블에서 이름, 입사일, 오늘 날짜 조회
...
-- <컬럼 별칭 지정>
-- 1. SELECT 조회 결과 집합인 RESULT SET에 출력되는 컬럼명을 지정
...
-- EMPLOYEE 테이블에서 보너스가 없는 사원의 이름ㅡ 보너스 조회
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
WHERE BONUS IS NULL;
 ...
 * 
 * ** 유의 사항 : SELECT문 해석 시 가장 마지막에 해석된다!
 * 
 * [해석 순서 매우 중요!]
 * 3. SELECT
 * 1. FROM
 * 2. WHERE
 * 4. ORDER BY 컬럼명 | 별칭 | 컬럼순서 [ACS(오름차순) | DESC(내림차순)] [NULLS FIRST | LAST]
 * 																		      DEFAULT                                    
 */
...
SELECT EMP_NAME "이름", HIRE_DATE "입사일" -- 2번해석 
FROM EMPLOYEE -- 1번 해석
ORDER BY "입사일"; -- 3번 해석
-- SELECT문의 해석 순서가 빠르기 때문에 별칭이 적용된 것을 ORDER BY 해석 시 인지 가능
